...
class AESImageEncryptor:
    def __init__(self, key=None):
        ...
        """
        self.key = key if key else get_random_bytes(16)  # AES 支持 16、24、32 字节的密钥
    ...
    def encrypt_image(self, image):
        ...
        return encrypted_image, (row_orig, column_orig, depth_orig), cipher.iv  # 返回 IV（初始化向量）

    def decrypt_image(self, encrypted_image, original_shape, iv):
        """
        ...
        # 创建解密器
        cipher = AES.new(self.key, AES.MODE_CBC, iv)  # 使用 CBC 模式并提供 IV
    ...
    def process_images(self, input_image_path, encrypted_image_path, decrypted_image_path):
        ...
        encrypted_image, original_shape, iv = self.encrypt_image(img)
        ...
        print("开始解密图像...")
        start_time = time.perf_counter()
        decrypted_image = self.decrypt_image(encrypted_image, original_shape, iv)
        end_time = time.perf_counter()
        ...
        for sigma in noise_levels:
            ...
            # 加密噪声图像
            encrypted_noisy_image, original_shape, iv = self.encrypt_image(noisy_image)

            # 解密噪声图像
            decrypted_noisy_image = self.decrypt_image(encrypted_noisy_image, original_shape, iv)
